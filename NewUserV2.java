...
public class NewUserV2 {
	...
	public void newScreen(LoadSaveDatabaseV3 stuff) {
		...
		try {
			NewUserV2 window = new NewUserV2();
			window.open(stuff);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		btnConfirm.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if(txtFirstName.getText().equals("") || txtMiddleName.getText().equals("") || txtLastName.getText().equals("") || 
					...
					if(cmbUserType.getSelectionIndex() == 0)
					{
						Administrators tempAd = new Administrators();
						tempAd.setFirst(txtFirstName.getText());
						tempAd.setMiddle(txtMiddleName.getText());
						tempAd.setLast(txtLastName.getText());
						tempAd.setPass(txtPassword.getText());
						tempAd.setID(txtUsername.getText());
						database.AdministratorList.add(tempAd);
					}else if (cmbUserType.getSelectionIndex() == 1)
					{
						Professors tempProf = new Professors();
						tempProf.setFirst(txtFirstName.getText());
						tempProf.setMiddle(txtMiddleName.getText());
						tempProf.setLast(txtLastName.getText());
						tempProf.setPass(txtPassword.getText());
						tempProf.setID(txtUsername.getText());
						database.ProfessorList.add(tempProf);
					}else{
						Students tempStud = new Students();
						tempStud.setFirst(txtFirstName.getText());
						tempStud.setMiddle(txtMiddleName.getText());
						tempStud.setLast(txtLastName.getText());
						tempStud.setPass(txtPassword.getText());
						tempStud.setID(txtUsername.getText());
						database.StudentList.add(tempStud);
					}
